...
#include <stdio.h>
#include <string.h>
#include "app_district_table_mem.h"
#include "app_mem.h"

static int mem_app_district_table_init(void);
static int mem_app_district_table_search(void* d);
static int mem_app_district_table_select(void* d);
static int mem_app_district_table_update(void* d);
static int mem_app_district_table_insert(void* d);
static int mem_app_district_table_delete(void* d);
static int mem_app_district_table_destroy(void);

static cJSON* district_table_array;
pthread_mutex_t mem_app_district_table_lock;

const app_mem_func_t mem_app_district_table = {
    mem_app_district_table_init,
    mem_app_district_table_search,
    mem_app_district_table_select,
    mem_app_district_table_update,
    mem_app_district_table_insert,
    mem_app_district_table_delete,
    mem_app_district_table_destroy
};

static int mem_app_district_table_init(void)
{
	pthread_mutex_init(&mem_app_district_table_lock, NULL);
	/*build Json stored in memory*/
	district_table_array = cJSON_CreateArray();
	if(NULL == district_table_array)
    {
        printf("district_table_array NULL\n");
        return -1;
    }
...
static int mem_app_district_table_search(void* d)
{
	...
	int num                     = 0;
	cJSON* data_obj             = NULL;
    app_district_table_tb member_data  = {0};
    ...
    app_district_table_mem_t* data = (app_district_table_mem_t*)d;

	pthread_mutex_lock(&mem_app_district_table_lock);
    
    num = cJSON_GetArraySize(district_table_array);
    for(i = 0; i < num; i++)
    {
    	data_obj           = cJSON_GetArrayItem(district_table_array, i);
	...
	pthread_mutex_unlock(&mem_app_district_table_lock);
...
static int mem_app_district_table_select(void* d)
{
    ...
    {
    	ret = -1;
    	printf("mem_district_table_dev_select not found\n");
    }
...
static int mem_app_district_table_insert(void* d)
{
    ...
    cJSON_AddItemToArray(district_table_array, data_obj);
...
static int mem_app_district_table_delete(void* d)
{
    ...
    for(i = 0; i < num; i++)
    {
        ...
        if(strcmp(data->member_data->scen_name->valuestring, member_data.scen_name->valuestring) == 0)
        {
        	cJSON_DeleteItemFromArray(district_table_array, i);
        	break;        
        }
    }
...
static int mem_app_district_table_destroy(void)
{
	cJSON_Delete(district_table_array);
	pthread_mutex_destroy(&mem_app_district_table_lock);

	return 0;
}

int app_district_table_mem_print(void)
{
	char * p = cJSON_PrintUnformatted(district_table_array);
    if(NULL == p)
    {    
